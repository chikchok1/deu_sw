...
public class ReservationchangeviewController {
    ...
    view.getCancelButton().addActionListener(e -> {
        ...
        DefaultTableModel model = (DefaultTableModel) view.getReservationTable().getModel();
        String selectedId = (String) model.getValueAt(selectedRow, 0); // 예약 ID
        if (selectedId == null || selectedId.trim().isEmpty()) {
        JOptionPane.showMessageDialog(view, "유효한 예약 ID가 없습니다.");
        return;
        }
        // ReserveClass.txt에서 해당 예약ID 삭제
        File originalFile = new File("data/ReserveClass.txt");
        ...
        try (
            ...
            JOptionPane.showMessageDialog(view, "파일 처리 중 오류: " + ex.getMessage());
            return;
        }

        if (!originalFile.delete() || !tempFile.renameTo(originalFile)) {
            JOptionPane.showMessageDialog(view, "파일 덮어쓰기 실패");
            return;
        }
        // ChangeRequest.txt에서도 삭제
         File changeFile = new File("data/ChangeRequest.txt");
         File tempChangeFile = new File("data/ChangeRequest_temp.txt");

        try (BufferedReader reader = new BufferedReader(new FileReader(changeFile));
        BufferedWriter writer = new BufferedWriter(new FileWriter(tempChangeFile))) {
        String line;
        while ((line = reader.readLine()) != null) {
        if (!line.startsWith(selectedId + ",")) {
            writer.write(line);
        ...
        changeFile.delete();
        tempChangeFile.renameTo(changeFile);
         } catch (IOException ex) {
        JOptionPane.showMessageDialog(view, "변경 요청 삭제 중 오류 발생: " + ex.getMessage());
        }
